...
class ClientVueron01(WebSocketWidget):
    ...
    def set_change_progressbar_tx(self, running=False):
        other = getattr(self.parent, 'client_vueron_02', None)
    ...
    def _parse_message(self, message):
        ...
        if not result:
            self.parser.notice.emit('Vueron_01 >> Invalid Json Data')
        ...
        if not timestamp_data:
            self.parser.notice.emit('Vueron_01 >> Missing trID')
            return
        try:
            ...
            self.parser.notice.emit('Vueron_01 >> Invalid trID format')
            return
